...
| Test | Modification réalisée | Tables modifiées | Colonnes modifiées | Observation | Interprétation |
|---|---|---|---|---|---|
| 0 | État initial | Aucune | Aucune | Baseline de référence générée (170 tables régissant l'écosystème IA et FDO) sans aucune donnée métier instanciée | Présence de nombreuses tables système Autodesk (préfixées par `TB_`) et OGC (`geometry_columns`, `spatial_ref_sys`) générées par défaut à l'initialisation. |
| 1 | Ajout d'une classe (`TEST_CLASSE_01`) | `TB_DICTIONARY`, `TB_ATTRIBUTE`, `fdo_columns`, `TB_RULE_BASE`, `TB_SEQUENCE_EMULATION`, tables UI (`TB_GN_*`, `TB_SETTINGS`) | `TEST_CLASSE_01` (table créée, + 5 triggers), `FID` (enregistré) | Création immédiate de la table physique et de ses triggers, ajout du métadonnier `FID`, et impact lourd paramétrique UI | `TB_DICTIONARY` est le catalogue maître. Toute classe implique la création d'une table SQL réelle, pilotée par une clé primaire indexée (`FID`). |
| 2 | Ajout d'un attribut texte (`TEST_ATTRIBUT_01`) | `TEST_CLASSE_01` (ALTER TABLE), `TB_ATTRIBUTE`, `fdo_columns`, `TB_SEQUENCE_EMULATION`, `TB_GN_FLYIN_USER` | `TEST_ATTRIBUT_01` (colonne créée, type VARCHAR) | Nouvelle colonne dimensionnée (VARCHAR2) physiquement ajoutée sur le SQL parent + catalogage FDO/UI | Chaque attribut modifie la structure DDL cible. Le paramétrage natif définit le descripteur formel (`fdo_data_type: 9`) en base. |
| 3 | Ajout d'un attribut numérique (`TEST_ATTRIBUT_02`) | `TEST_CLASSE_01` (ALTER TABLE), `TB_ATTRIBUTE`, `fdo_columns`, `TB_SEQUENCE_EMULATION` | `TEST_ATTRIBUT_02` (colonne créée, type INTEGER) | Colonne (`INTEGER`) physiquement instanciée ; le système modifie le code type du système FDO FDO pour correspondre à Number | Le typage est double : logique vers l'application via `fdo_columns` (`type: 7`), et contrainte rigide SQL en backend (`INTEGER(10)`). |
| 4 | Valeur par défaut (`TEST_ATTRIBUT_03`) | `TEST_CLASSE_01` (ALTER TABLE), `TB_ATTRIBUTE`, `fdo_columns`, `TB_SEQUENCE_EMULATION` | `TEST_ATTRIBUT_03` (colonne créée avec `DEFAULT`) | La table instancie explicitement la contrainte DDL SQLite `DEFAULT 0`, ce que le catalogue n'évoque pas du tout en isolation | L'outil repose directement sur la mécanique fonctionnelle SQLite pour gérer les valeurs par défaut, plutôt qu'une couche logicielle externe. | Oui |
| 5 | Attribut obligatoire (`TEST_ATTRIBUT_05`) | `TEST_CLASSE_01` (ALTER TABLE), `TB_ATTRIBUTE`, `fdo_columns`, `TB_SEQUENCE_EMULATION` | `TEST_ATTRIBUT_05` (colonne créée avec `NOT NULL`) | L'option obligatoire génère logiquement le tag NOT NULL, non renseigné dans des index descriptifs booléens d'obligations | Comme pour les valeurs par défaut, l'intégrité "Obligatoire" est directement déléguée au moteur relationnel SQLite. | Oui |
| 7 | Nouvelle classe géométrique Point (`TEST_CLASSE_GEO_01`) | `TEST_CLASSE_GEO_01` (créée, + triggers), `geometry_columns`, `TB_DICTIONARY`, `TB_ATTRIBUTE`, `fdo_columns`, `TB_RULE_BASE`, `TB_SETTINGS` | `TEST_CLASSE_GEO_01` (créée), `FID`, `Z`, `ORIENTATION`, `GEOM`, `QUALITY` (colonnes physiques créées) | Injection automatique d'un bloc de champs spatiaux (GEOM, point quality, rotation) ; F_CLASS_TYPE identifié à 'P' (Point) | Respecte les standards SIG (`geometry_columns`). L'architecture embarque automatiquement de riches contraintes visuelles au sein de la table des Points. | Oui |
| 8 | Classe géométrique Ligne (`TEST_CLASS_GEO_02`) | `TEST_CLASS_GEO_02` (créée, + triggers), `geometry_columns`, `TB_DICTIONARY`, `TB_ATTRIBUTE`, `fdo_columns`, `TB_RULE_BASE`, `TB_SEQUENCE_EMULATION` | `TEST_CLASS_GEO_02` (créée), `FID`, `GEOM`, `LENGTH` (colonnes physiques créées) | Les métadonnées diffèrent des points, la `geometry_type` vaut 2 et impose automatiquement un espace mémoire de suivi pour la longueur | Le changement du code F_CLASS_TYPE ('L') impacte concrètement le DDL applicatif : un attribut de performance analytique (`LENGTH`) est déployé systématiquement. | Oui |
